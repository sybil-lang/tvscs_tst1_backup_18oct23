    ...
    $("#datetimepicker"+scounter).on("dp.change", function(e) {
		...
		static_date =	$('#datetimepicker'+scounter+' input').val();
		 ...
		 // if()
		  ...
		  console.log("month_difference",diff_mon);
		  if(diff_mon <=3  && diff_mon >= 0){
			  $("#loader").removeClass("hidden");
				if(dealerProduct=="CD")
			    {
					   	if(ecounter==13 && scounter==12){
					   	var url = "/cc/DealerCustom/reloadPaymentDetails?start_date="+static_date+"&end_date="+statice_date+"&dealer_code="+dealerCode;
					   	ddtable.ajax.url(url).load();
					   	}
					   	else if(ecounter==7 && scounter==6){
					   		var url = "/cc/DealerCustom/reloadDealerDisbursalRestData?start_date="+static_date+"&end_date="+statice_date+"&dealer_code="+dealerCode;
						   	ddtable.ajax.url(url).load();
					   	}
					   	else if((ecounter==9 && scounter==8)||(ecounter==11 && scounter==10)){
				   	 ...
				   	 if(ecounter==7 && scounter==6){
				   	 		var url = "/cc/DealerCustom/reloadDealerDisbursalRestData?start_date="+static_date+"&end_date="+statice_date+"&dealer_code="+dealerCode;
						   	ddtable.ajax.url(url).load();
				   	 }
				   	 else if(ecounter == 25 && scounter == 24){
				   	 	var url = "/cc/AjaxCustom/payment_msme?start_date="+static_date+"&end_date="+statice_date+"&agreement_no="+agreement;
				   	 	msme_payment_details.ajax.url(url).load();
				   	 }
				   	 else{
				   	 		var url = "/cc/DealerCustom/reloadSalesRestData?method="+methodName+"&start_date="+static_date+"&end_date="+statice_date+"&dealer_code="+dealerCode;
				   			ddtable.ajax.url(url).load();
    ...
    $("#datetimepicker"+ecounter).on("dp.change", function(e) {
		...
		statice_date =	$('#datetimepicker'+ecounter+' input').val();
		 //alert();
		  ...
		  console.log("month_difference2",diff_mon);
