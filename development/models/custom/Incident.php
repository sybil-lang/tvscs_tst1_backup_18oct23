 ...
 */
class Incident extends \RightNow\Models\PrimaryObjectBase
{
    /**
    ...
    // public function updatenocreatebycontact(){
    //     // 221203-000012
    //     // $incident = $this->getBlank()->result;
    //     $incident = \RightNow\Connect\v1_3\Incident::fetch("221203-000012");
    ...
    public function create(array $formData, $smartAssist = false)
    {
        ...
        if ($formData['Incident.Primary']) {
            if ($contact = $this->getContact()) {
                $incident->PrimaryContact = $contact;
            }
            unset($formData['Incident.Primary']);
        }
        // print_r($formData['Incident.CustomFields.c.imei_num']);

        if ($formData['Incident.CustomFields.c.imei_num']->value) {

            $incident->CustomFields->c->imei_num = $formData['Incident.CustomFields.c.imei_num']->value;
        }
        if ($formData['Incident.PrimaryContact']) {
            if ($formData['Incident.PrimaryContact'] instanceof Connect\Contact) {
                $incident->PrimaryContact = $formData['Incident.PrimaryContact'];
            } else if (
                (is_int($formData['Incident.PrimaryContact']) || ctype_digit($formData['Incident.PrimaryContact']))
        ...
        try {
            $c_id = $this->CI->session->getProfileData("c_id");
            $incident->CustomFields->CO->CreatedByContact = RNCPHP\Contact::fetch($c_id);
            if ($incident->PrimaryContact->ID != $c_id) { // If Incident.Contact is not same as the Portal-Logged-In-User, then set Incident.OtherContacts=Portal-Logged-In-User
                $incident->OtherContacts[0] = RNCPHP\Contact::fetch($c_id);
            }
            $loanParam = \RightNow\Utils\Url::getParameter('loanid');
            if (!empty($loanParam)) {
                //\RightNow\Connect\v1_3\CO\Loan::fetch($agg_no);
                list($loan_no,) = explode("_", $loanParam);
                $incident->CustomFields->CO->Loan = \RightNow\Connect\v1_3\CO\Loan::fetch($loan_no);
            }
            $incident->save();
            ...
            //	echo $incident->ReferenceNumber; die;
        } catch (\Exception $e) {
        ...
        try {
            ...
            $hookData = array(
                'knowledgeApiSessionToken' => $this->getKnowledgeApiSessionToken(),
                'smartAssistantToken' => $smartAssistantToken,
     ...
     * @return Connect\Incident|null Updated incident object containing the incident or error messages if the incident wasn't updated
     */
    public function empIncUpdate($incidentID, array $formData)
    {
        $incident = RNCPHP\Incident::fetch($incidentID);

        $loggedInContactID = $this->CI->session->getProfileData('contactID');
        $loggedInContact = RNCPHP\Contact::fetch($loggedInContactID);
        $loggedInContactName = $loggedInContact->Name->First . " " . $loggedInContact->Name->Last;

        $t_count = count($incident->Threads);
        if ($t_count == 0)
            $incident->Threads = new RNCPHP\ThreadArray();
        $incident->Threads[$t_count] = new RNCPHP\Thread();
        $incident->Threads[$t_count]->EntryType = new RNCPHP\NamedIDOptList();
        $incident->Threads[$t_count]->EntryType->ID = 1; // Customer
        $incident->Threads[$t_count]->Channel = new RNCPHP\NamedIDLabel();
        $incident->Threads[$t_count]->Channel->ID = 6; // Service Web
        $incident->Threads[$t_count]->Text = "Updated By Employee: " . $loggedInContactName . "\n\n" . $formData["Incident.Threads"]->value;

        if ($incident->StatusWithType->Status->ID == 2) { // Closed
            $incident->StatusWithType->Status->ID = 102; // Re-Open
        } else {
            $incident->StatusWithType->Status->ID = 8; // Response Awaited
        }
        $incident->CustomFields->c->incident_id = $formData["Incident.CustomFields.c.incident_id"]->value;
        $incident->CustomFields->c->do_you_want_a_response->ID = $formData["Incident.CustomFields.c.do_you_want_a_response"]->value;
        if ($formData["Incident.CustomFields.c.do_you_want_a_response"]->value == "174") {
            $incident->StatusWithType->Status->ID = 2; // Closed
        }

        $this->createAttachmentEntry($incident, $formData["Incident.FileAttachments"]->value);
