 ...
 */
class Prodcat extends \RightNow\Models\Prodcat {
    const MAX_LEVEL = 2;
     ...
     */
    public function getFormattedTree($dataType, array $selectedChain, $linkingOn = false, $linkingProductID = null, $maxLevel = 6, $empCustHelpdeskReportId) {
        ...
        if($dataType == "Category"){
            $empCustHelpdeskCategoriesReport = \RightNow\Connect\v1_3\AnalyticsReport::fetch($empCustHelpdeskReportId);
            $empCustHelpdeskCategoriesReportRun = $empCustHelpdeskCategoriesReport->run();
            $empCustHelpdeskCategoriesReportRunCount = $empCustHelpdeskCategoriesReportRun->count();
            $customProdcatTree = array();
            for($i = 0; $i<$empCustHelpdeskCategoriesReportRunCount; $i++){
                $row = $empCustHelpdeskCategoriesReportRun->next();
                $customProdcatTree[0][$i]["id"] = (int)$row["Category_Id"];
                $customProdcatTree[0][$i]["label"] = $row["Category"];
                $customProdcatTree[0][$i]["hasChildren"] = false;
                $customProdcatTree[0][$i]["selected"] = false;
            }
            return $this->getResponseObject($customProdcatTree, 'is_array', array(), $additionalWarning ?: array());
        }
        else{
