 ...
 */
class Field extends \RightNow\Models\Base {
   ...
   */
  public function resetDealerPassword($formData, $resetCredentials) {
  ...
  public function sendForm($formData, $listOfUpdateIDs = array(), $smartAssistant = false) {
    ...
    if ($presentFields['contact']) {
      ...
      //Incident create email only (and can optionally contain other contact fields)
        ...
        $existingContact = $this->CI->model('Contact')->lookupContactByEmail(
                        $formData['Contact.Emails.PRIMARY.Address']->value, $formData['Contact.Name.First'] ? $formData['Contact.Name.First']->value : null, $formData['Contact.Name.Last'] ? $formData['Contact.Name.Last']->value : null
                )->result;
        if ($existingContact) {
    ...
    if ($presentFields['incident'] && !$presentFields['socialquestion']) {
      if ($incidentIDToUpdate = $listOfUpdateIDs['i_id']) {
        ...
        //	$formData['Incident.CustomFields.CO.CreatedByContact'] =  RNCPHP\Contact::fetch($c_id);
        $formData['Incident.Primary'] = 0;
        $loanParam = \RightNow\Utils\Url::getParameter('loanid');
        if (!empty($loanParam)) {
          //\RightNow\Connect\v1_3\CO\Loan::fetch($agg_no);
          list($loan_no, $cid) = explode("_", $loanParam);
          if (!empty($cid)) {
            $formData['Incident.PrimaryContact'] = $this->CI->model('Contact')->get($cid)->result;
          } else {
            $ci_id = $this->CI->session->getProfileData("c_id");
            $formData['Incident.PrimaryContact'] = $this->CI->model('Contact')->get($ci_id)->result;
          }
        } else {
          $dealerid = \RightNow\Utils\Url::getParameter('dealerid');
          if (!empty($dealerid)) {
            $formData['Incident.PrimaryContact'] = $this->CI->model('Contact')->get($dealerid)->result;
          } else {
            $formData['Incident.Primary'] = 1;
          }
        }

        //	$formData['Incident.CustomFields.CO.Loan'] =  \RightNow\Connect\v1_3\CO\Loan::fetch($loan_no);
        //		echo $agg_no;
        ...
        //	print_r($formData);
        $return = $this->CI->model('custom/Incident')->create($formData, $smartAssistant);
        $action = 'Created';
      }
   ...
   * @return array Details of the form submit action including any errors that might have occured
   */
  public function sendFormEmpIncUpdate($formData, $listOfUpdateIDs = array(), $smartAssistant = false) {
    if ($tokenError = $this->verifyFormToken())
      return $tokenError;
    $userIsLoggedIn = Framework::isLoggedIn();
    $actions = array();
    ...
    // Either Incident or Question. There's (probably) not a valid use-case to support both at the same time.
    if ($presentFields['incident'] && !$presentFields['socialquestion']) {
      $return = $this->CI->model('custom/Incident')->empIncUpdate($formData["Incident.CustomFields.c.incident_id"]->value, $formData);
      $action = 'Updated';
      $actions += $processActions('incident', $action, $return);
    } else if ($presentFields['socialquestion'] && !$presentFields['incident']) {
      if ($questionIDToUpdate = $listOfUpdateIDs['qid']) {
  ...
  private function verifyFormToken($tokenName = 'f_tok', $tokenSeed = 0) {
    if (!Framework::isValidSecurityToken($this->CI->input->post($tokenName), $tokenSeed)) {
      return $this->getResponseObject(array('redirectOverride' => '/app/error/error_id/5', 'sessionParam' => \RightNow\Utils\Url::sessionParameter()), 'is_array', Config::getMessage(FORM_SUBMISSION_TOKEN_MATCH_EXP_MSG)
      );
    }
  }
