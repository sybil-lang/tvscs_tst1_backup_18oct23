...
class Login extends \RightNow\Models\Base
{
  ...
  function sendOTPEmail($email, $sessionID, $widgetID, $url)
  {
    ...
    if ($report_id > 0) {
      ...
      if (strlen($contact_id) and $contact_id > 0) {
        ...
        $sms_message = 'Dear Customer, ' . $otp . ' is your OTP to access TVS Credit portal. Please use this OTP within 15 min time. Do not disclose OTP to anyone. For further details call 1800 103 5005 - TVS Credit';
  ...
  function sendOTP($mnumber, $sessionID, $widgetID, $url, $divcheck)
  {
    ...
    if ($report_id > 0) {
      ...
      if (strlen($contact_id) and $contact_id > 0) {
        ...
        $c_loantype = $contact->CustomFields->c->loan_type->LookupName;
        // echo "-Loan Type-" .$c_loantype. " -divcheck- ". $divcheck. " -- "; exit("EXIT");
        if (($c_loantype == "Retail" || $c_loantype == null || $loanType == "Both") && $divcheck == "rn_LoginOtp_7_Submit_OTP") {

          // return $this->getResponseObject("You are not authorized to login...", 'is_array');
          $result['message'] = 'You are not authorized to login....';
          $result['success'] = 0;
          return $this->getResponseObject($result, 'is_array');
        }

        if (($c_loantype == "MSME") && $divcheck == "rn_LoginOtp_6_Submit_OTP") {
        ...
        if (isset($responseData->ErrorCode) && $responseData->ErrorCode == 1) {
          ...
          $sms_message = 'Dear Customer, ' . $otp . ' is your OTP for TVS Credit portal. This is valid for 15 mins. Do not disclose OTP to anyone. For assistance, call 18001035005. TVS Credit';
  ...
  public function getEmployeeProfile($username, $password, $sessionID, $custType)
  {
    ...
    $url = "https://userloginuat.tvscredit.com/Service.asmx"; //'https://sso.tvscredit.com/Service.asmx';
    $this->CI->load->library("Nusoap_lib");
    //  $client = $this->CI->nusoap_lib->nusoap_client( $url, 'wsdl' );
    //  require_once( '/cgi-bin/tvscs.cfg/scripts/cp/customer/development/libraries/nusoap/nusoap.php' );
    $body = '<?xml version="1.0" encoding="utf-8"?>
  ...
  public function getMSMEProfile($username, $password, $sessionID, $custType)
  {
    ...
    curl_setopt_array($curl, array(
      CURLOPT_URL => 'https://msmecfpguat.tvscredit.com/MSMECFPG/msmecf/loginauth',
    ...
    if ($response->status == 'S') {
      ...
      $res = Connect\ROQL::query("SELECT Contact.Login FROM Contact C WHERE C.CustomFields.c.user_name = '" . $username . "' AND Contact.ContactType = 1 AND ( Contact.CustomFields.c.loan_type = 270 OR Contact.CustomFields.c.loan_type = 269 )")->next();
      if ($res->count() > 0) {
