 <rn:meta title="Lead Creation" template="dealer_header.php" login_required="true" force_https="true" />
<style type="text/css">
.rn_Body{
min-height: 800px;
}
...
$tvsapiurl = $msg->Value;
//http://leadsservice.tvscs.co.in/TVSCSLMSAPI.svc?wsdl
									...
									<div class="sf-steps-form sf-radius"> 
										...
										<ul class="sf-content" id="last_step"> <!-- form step tree --> 
											...
											<li>		
												...
												<li>	
													<div class="sf_columns column_2">
													  <label for="ProductCode" class="sf-select" > Product
																	  <select name="loan_type" id="loanList" data-required="true" >
																		<option value="">Select Loan Type</option>
																		<!-- <?php
	// if ( count( $response_decoded ) > 0 ) {
	// 	foreach ( $response_decoded as $display_text ) {
	// 		$selected = ( $display_text[ 'szdesc' ] == $this->input->post( 'ProductCode' ) ) ? ' selected="selected"' : null;
	// 		echo '<option value="' . $display_text[ 'szproductcode' ] . '" ' . $selected . '>' . $display_text[ 'szdesc' ] . '</option>';
	// 	}
	// }
																							?> -->
																	  </select>
									...
									<input id="ChannelCode" name="ChannelCode" type="hidden" value="DP">
...
<script>
	$(document).ready(function(e) {
		...
		$(".stepsForm").stepsForm({
			...
			posturl			:'/cc/DealerCustom/saveLeadData',
			theme			:'blue',
		}); 
});
</script>
