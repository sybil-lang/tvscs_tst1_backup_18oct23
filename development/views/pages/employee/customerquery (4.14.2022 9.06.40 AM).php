...
$(document).ready(function(){
   ...
   //$('select[name="Incident.CustomFields.c.preferred_address"]').val(254).change();
                ...
                if(cat=="Excess Refund")
										    {
										    	cat=56;
                                                     ...
                                                     $.ajax({
                                                                ...
                                                                async: false,
                                                                data: {   ag_no : ag_no,  cat : cat,mobile:mobile,dob:dob,customername:customername,value:value,Question:Question,subjectt:subjectt},
                                                                success: function(data) {
                                                                                                         dd=JSON.parse(data)  
                                                                                                         console.log(dd);
                                                                                                          bootbox.alert(dd.ReturnMessage,function(){ location.reload(); });
                                                                                                            die();
                                                                                         }
                                                             });
        ...
        {
					         e.preventDefault();
					         // if($('select[name="Incident.CustomFields.c.preferred_address"]').css("display")=="block" && $('select[name="Incident.CustomFields.c.preferred_address"]').val()=='')
					         // {
					         // 	alert("Please select the Preferred address");
					         // 	die();
					         ...
					         var categories = [<?php echo $category_list; ?>];
					  			if(categories.includes(cat.trim()) == true){
					  			 var value= document.getElementsByName('Incident.CustomFields.c.preferred_address')[0].value
								       ...
								       if(value=='252' && !(daddress))
							  			{
							  				die();

							  			}
							  			if(value=='253' && (!(daddress) || !(pin)))
							  			{
							  				die();
...

			           {
                                                    ...
                                                    if(cat==56)
                                                 {
                                                       $.ajax({
                                                        ...
                                                        url: "/cc/AjaxCustom/excess_refund",
                                                        async: false,
                                                        data: {   ag_no : ag_no,  cat : cat,mobile:mobile,dob:dob,customername:customername,Question:Question,subjectt:subjectt},
                                                        success: function(data) {
                                                       dd=JSON.parse(data)  
                                                       console.log(dd);
                                                       if(dd.ReturnID==1001)
