        ...
        <script>
            ...
            $(document).ready(function() {
                ...
                $('#search').click(function() {
                    ...
                    $('.page-link-m').each(function(index) {
                        $(this).html(index + 1);
                    });
                    $('.page-link-m').each(function() {
                        $(this).attr('onclick', 'page_show_search(' + $(this).html() + ')');
                    });
                    page_number_search = 1;
                    last_page_flag_search = false;
...
$.post("/cc/AjaxCustom/GetTokenListApiForTmsDashboard", {
    ...
    console.log(data);
    try {
        var main_data = JSON.parse(somedata).ReturnOutput;
        console.log(main_data);
        
        // Convert the data to an array of arrays that can be used by SheetJS
        var rows = [];
        for (var i = 0; i < main_data.length; i++) {
            rows.push(Object.values(main_data[i]));
        }
        
        // Convert the data to a CSV file using SheetJS
        var csv = XLSX.utils.sheet_to_csv(XLSX.utils.aoa_to_sheet(rows));
        
        // Create a temporary link and trigger a download of the CSV file
        ...
        console.log(err.message);
                ...
                $.post("/cc/AjaxCustom/GetTokenListApiForTmsDashboard", {
                    ...
                    try {
                        ...
                        if (last_page_flag == true) {
                            ...
                            console.log("page_numb", page_number);
                        ...
                        for (i = 0; i < loopvalue; i++) {
                            ...
                            var AgreementNumber = main_data['ReturnOutput'][i]['AgreementNumber'];
                            var AgreementNumberDisplay = AgreementNumber ? AgreementNumber : "--";

                            var TokenRequestId = main_data['ReturnOutput'][i]['TokenRequestId'];
                        ...
                        if (end_index + 1 == total_records || end_index > total_records) {
            ...
            function SearchingByCat() {
                ...
                var from_date_str = $('#from_date').val();
                var to_date_str = $('#to_date').val();
                ...
                if (!from_date_str && !to_date_str && !branch_code && !ecode_code && !uname_code && !status_code && !product_code && !agreement_code) {
                    return;
                }
                ...
                if (from_date_str && to_date_str) {
                    ...
                    // Convert the Date objects to ISO strings in UTC time zone
                    var from_date_iso = new Date(Date.UTC(from_date.getFullYear(), from_date.getMonth(), from_date.getDate())).toISOString().substr(0, 10);
                    var to_date_iso = new Date(Date.UTC(to_date.getFullYear(), to_date.getMonth(), to_date.getDate())).toISOString().substr(0, 10);

                    console.log(from_date); // Thu Mar 02 2023 00:00:00 GMT+0530 (India Standard Time)
                    console.log(to_date); // Thu Mar 02 2023 00:00:00 GMT+0530 (India Standard Time)
                    console.log(from_date_iso); // "2023-03-01"
                    console.log(to_date_iso); // "2023-03-01"
                }
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if (response_data["ReturnMessage"] == "No Data Found.") {
                            ...
                            var loopvalue = response_data['ReturnOutput'].length;

                            for (var i = totalpage; i < pageItems_m.length; i++) {
                                pageItems_m[i].style.display = "none";
                            }
                            if (totalpage == page_number_search) {
                                last_page_flag_search = true;
                            }
                            console.log("page_number_search", page_number_search);
                            ...
                            console.log("page_number_search", page_number_search);
                            if (page_number_search == 1) {
                                const liElements = document.querySelectorAll('.page-items');
                                liElements.forEach(li => {
                                    const defaultValue = li.getAttribute('data-default-value');
                                    const a = li.querySelector('a');
                                    a.innerHTML = defaultValue;
                                });
                            }
                            var j = startindex;
                            ...
                            const lastButton = document.getElementById("last_button");
                            const nextButton = document.getElementById("next_button");
                            if (endindex + 1 == totalrecords || endindex > totalrecords) {
                                lastButton.classList.add("disabled");
                                nextButton.classList.add("disabled");
                            } else {
                                lastButton.classList.remove("disabled");
            ...
            function next_page() {
                ...
                // Get all the li elements with class "page-items"
                const pageItems = document.querySelectorAll('li.page-items');
                var pageLinks = document.getElementsByClassName("page-link-m");

                // Loop through each li element and check if it has the "active" class
                for (let i = 0; i < pageItems.length; i++) {
                    if (pageItems[i].classList.contains('active')) {
                        const activeLiNumber = i;
                        console.log(`The active li element is number ${activeLiNumber}`);
                        // This li element has the "active" class
                        pageItems[i].classList.remove('active'); // remove "active" class from the current li element
                        let newActiveLi;
                        if (i < pageItems.length - 1) {
                            // Add "active" class to the next li element
                            newActiveLi = pageItems[i + 1];
                            newActiveLi.classList.add('active');
                        } else {
                            // If the last li element has the "active" class, keep it active
                            newActiveLi = pageItems[i];
                            newActiveLi.classList.add('active');
                        }
                        // If the active li element is the 5th element, update the inner HTML of all li elements
                        if (i === 4) {
                            // Loop through all the li elements and update their inner HTML
                            for (let j = 0; j < pageLinks.length; j++) {
                                const currentNumber = parseInt(pageLinks[j].innerHTML);
                                pageLinks[j].innerHTML = currentNumber + 5;
                            }
                            // Add "active" class to the first li element
                            if (!pageItems[0].classList.contains('active')) {
                                pageItems[4].classList.remove('active');
                                pageItems[0].classList.add('active');
                                newActiveLi = pageItems[0];
                            }
                        }
                        console.log(newActiveLi.querySelector('a').innerHTML);
                        let next_page_number = newActiveLi.querySelector('a').innerHTML;
                        page_number = next_page_number;
                        break; // exit the loop since we only need to update one li element
                    }
                }
            ...
            function next_page_search() {
                ...
                for (let i = 0; i < pageItems.length; i++) {
                    if (pageItems[i].classList.contains('active')) {
                        ...
                        console.log(newActiveLi.querySelector('a').innerHTML);
                        let next_page_number = newActiveLi.querySelector('a').innerHTML;
                        page_number_search = next_page_number;
                        break; // exit the loop since we only need to update one li element
                    }
                }
                ...
                // var activeLi = document.querySelector(".page-items.active");
                // activeLi.classList.remove("active");

                // // Add active class to next li element
                // var nextLi = document.querySelector(".page-items[data-default-value='" + page_number_search + "']");
            ...
            function page_show_search(value) {
                ...
                for (var i = 0; i < pageLinks.length; i++) {
                    ...
                    if (i + 1 == value) {
                        pageLinks[i].parentElement.classList.add("active");
                        console.log("innerHTML", pageLinks[i].innerHTML);
                        page_number_search = pageLinks[i].innerHTML;
                    }
                }
                console.log("page_number_search", page_number_search);
                SearchingByCat();
                ...
                <div class="page_navigation">
                    <nav aria-label="Page navigation example">
                        <ul class="pagination">
                            ...
                            <li class="page-items active" data-default-value="1"><a class="page-link-m" href="javascript:void(0)" onclick="page_show(this.innerHTML)">1</a></li>
                            <li class="page-items" data-default-value="2"><a class="page-link-m" href="javascript:void(0)" onclick="page_show(this.innerHTML)">2</a></li>
                            <li class="page-items" data-default-value="3"><a class="page-link-m" href="javascript:void(0)" onclick="page_show(this.innerHTML)">3</a></li>
                            <li class="page-items" data-default-value="4"><a class="page-link-m" href="javascript:void(0)" onclick="page_show(this.innerHTML)">4</a></li>
                            <li class="page-items" data-default-value="5"><a class="page-link-m" href="javascript:void(0)" onclick="page_show(this.innerHTML)">5</a></li>
