      ...
      <script>
        $(document).ready(function() {
            ...
            $.post("/cc/AjaxCustom/GetTokenCountByStatusForReport", {
              ...
              function(data) {
                ...
                try {
                  ...
                  $('#all_tokens').html(data.ReturnOutput[0].CNT);
                  $('#open_tokens').html(open_tokens);
                  $('#closed_tokens').html(closed_tokens);
                  $('#service_requests').html(service_requests);
                } catch (err) {
          ...
          $("#closer").click(function() {
            ...
            var selectedref = getSelectedref();
            console.log(typeof(selectedTokens));
            console.log(selectedTokens);
            console.log(selectedTokens.length);
            if (selectedTokens.length > 0) {
              console.log("selectedTokens", selectedTokens);
              var commaSeparatedString = selectedTokens.join('');
              console.log("commaSeparatedString", commaSeparatedString);
              console.log(typeof(commaSeparatedString));
              // for (var i = 0; i < selectedTokens.length; i++) {
              //   intTokens.push(parseInt(selectedTokens[i]));
              // }
              var employeecode = <?php echo $employeeCode; ?>;
              bootbox.dialog({
                title: "Bulk Closure",
                  ...
                  confirm: {
                    ...
                    callback: function() {
                      ...
                      if (status === 'new_request') {
                        window.location.href = 'https://tvscs.custhelp.com/app/employee/walkin_report_edit_new';
        ...
        function generatetable() {
          ...
          $.post("/cc/AjaxCustom/GetTokenListApiForTmsDashboard", {
            ...
            try {
              ...
              for (var i = total_page; i < pageItems_m.length; i++) {
                // console.log("hi");
                pageItems_m[i].style.display = "none";
              }
              if (total_page == page_number || total_page < page_number) {
                last_page_flag = true;
              }
              ...
              for (i = 0; i < loopvalue; i++) {
                ...
                // Add the view icon
                html += "<a href='https://tvscs.custhelp.com/app/employee/walkin_reports_view/?token=" + main_data['ReturnOutput'][i]['Token'] + "'><i class='fa fa-eye' style='font-size:18px; color: #03a9f4; padding-right: 10px;'></i></a>";

                // Add the edit icon only if the status is not closed
                if (main_data['ReturnOutput'][i]['CaseStatus'] !== 'CLOSED') {
                  html += "<a href='https://tvscs.custhelp.com/app/employee/walkin_reports_edit/?token=" + main_data['ReturnOutput'][i]['Token'] + "'><i class='fa fa-edit' style='font-size:18px; color: #03a9f4; padding-right: 10px;'></i></a>";
        ...
        function generatedivtable(code_status) {
        ...
        function SearchByCat() {
          ...
          $.ajax({
            ...
            success: function(response) {
              ...
              if (response_data["ReturnMessage"] == "No Data Found." || response_data["ReturnOutput"] === null) {
                ...
                for (var i = startindex; i < response_data.ReturnOutput.length; i++) {
                  ...
                  } else if (response_data['ReturnOutput'][i]['CaseStatus'] === 'CAC') {
                    htmlresponse += "#1b8258; color: white; text-align: center;'>" + "NEW SERVICE REQUEST" + "</td>";
                  } else {
                    htmlresponse += "#1b8258; color: white; text-align: center;'>" + "OPEN" + "</td>";
                  }
                  ...
                  // Add the view icon
                  htmlresponse += "<a href='https://tvscs.custhelp.com/app/employee/walkin_reports_view/?token=" + response_data['ReturnOutput'][i]['Token'] + "'><i class='fa fa-eye' style='font-size:18px; color: #03a9f4; padding-right: 10px;'></i></a>";

                  // Add the edit icon only if the status is not closed
                  if (response_data['ReturnOutput'][i]['CaseStatus'] !== 'CLOSED') {
                    htmlresponse += "<a href='https://tvscs.custhelp.com/app/employee/walkin_reports_edit/?token=" + response_data['ReturnOutput'][i]['Token'] + "'><i class='fa fa-edit' style='font-size:18px; color: #03a9f4; padding-right: 10px;'></i></a>";
