	...
	<form id="rn_QuestionSubmit" method="post" action="/cc/ajaxRequest/sendForm/" class="styled" enctype="multipart/form-data">
		...
		<div class="row">
			<div class="col-md-6">
				...
				<!-- <div class="widgetfile  " id="widfile">
					<rn:widget path="input/FileAttachmentUpload" name="Incident.FileAttachments" />

				</div>

				<div class="ui-widget  hidden " id="infile">
...
<script>
	...
	$(document).ready(function() {
		...
		$('#rn_ProductCategoryInputEmpCustomer_29_Button_Visible_Text').on('DOMSubtreeModified', function() {
			...
			if (categories.includes(sel_val.trim()) == true) {
				...
				// let agreementNoInput = document.getElementById("agreement_no");
				// let agreementNoValue = agreementNoInput.value;
				// console.log('dfedwdfvredfv', agreementNoValue);

				var a = $('#agreement_no').val().split('#')
				var ag_no = a[2];
				console.log("ddfds", ag_no)
				// alert("ddfds", ag_no)
				...
				localStorage.setItem('agreement_number', ag_no);
				localStorage.setItem('customer_name', custname);
				localStorage.setItem('subject', subjectt);


				if (sel_val === "NOC request") {
					// if (categories.includes('NOC request')) {
					console.log("inside nocif");
					window.location.replace("https://tvscs--tst1.custhelp.com/app/employee/customerquerynoc");
	...
	$(document).ready(function() {
		...
		$('#rn_empFormSubmit_37_Button').on('click', function(e) {
			...
			if ((cat == 1891 || cat == 1812 || cat == 86 || cat == 77 || cat == 58 || cat == 56 || cat == 57 || cat == 1727) && ag_no)
								...
								buttons: {
									...
									Yes: {
										...
										callback: function(result) {
											...
											if (ResultRespons4.status.includes("ER") && ResultRespons4.mesage.includes("Challenge")) {
															...
															buttons: {
																...
																Yes: {
																	...
																	callback: function(result) {
																		...
																		if (ResultRespons4.status.includes("ER") && ResultRespons4.mesage.includes("Released")) {
