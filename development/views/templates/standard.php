...
,buttons.dataTables.min.css" />
 ...
 if($contact_id != "")
 {
	$msg2 = RightNow\Connect\v1_3\MessageBase::fetch(CUSTOM_MSG_Prospect_No_Instrument_Details_Page);
		$report_id1 = $msg2->Value;
	
	$filter1 = array('ContactID' => $contact_id);
	$IarcResponse = report_result($report_id1, $filter1);
	

		$j=0;
		$CountTransferredAg = 0;
        $arrcount = count($IarcResponse);
		// echo $arrcount;
		for($j=0; $j<$arrcount; $j++)
		{
			if($IarcResponse[$j]['Transferred Agreement'] == "Yes")
	    {
	        $CountTransferredAg ++;
	    ...
	    if($CountTransferredAg == $arrcount)
	    {
	    //  echo 'Blpck scenario';
		 $value = "1";
		...
		else if($CountTransferredAg > 0 && $CountTransferredAg < $arrcount)
		{
			$value = "multiple";
			echo "<script>localStorage.setItem('key', '$value');</script>";
		}
 ...
 //IARC
	...
	$notification_text .= '<div class="notification-item">
														<p class="item-info">Our toll free no. has been changed. New no. is 044-66-123456</p>
													  </div>
													  ';
			...
			<div class="container1">
					...
					<ul class="nav-menu navbar-right primary-menu">
						...
						<li id="dropacc">
							<div class="rn_LoginStatus">
								<rn:condition logged_in="true">
									...
									<rn:widget path="custom/login/AccountDropdown" subpages=" Profile Information > account/overview, #rn:msg:SUPPORT_HISTORY_LBL# > account/questions/list, #rn:msg:ACCOUNT_SETTINGS_LBL# > account/profile" />
								...
								<div class="dropdowns">
									...
									<ul class="dropdown-menu notifications" role="menu" aria-labelledby="dLabel">
										...
										<div class="notification-heading">
											...
											<h4 class="menu-title pull-right"><a href="<?php echo $site_url; ?>/app/customer/selfserviceview/notification/1">View all</a><i class="glyphicon glyphicon-circle-arrow-right"></i></h4>
										</div>
										<li class="divider"></li>
										<div class="notifications-wrapper scrollbar-auto" id="notification-window" style="height: fit-content;word-break: normal;white-space: normal;margin:10px;">
											...
											<?php
											echo "<hr/><br><h5>Dear " . $customer_name . ", You can now reach us on our new customer care number 044-66-123456, on Monday to Saturday, from 9 am to 7 pm.</h5>" . $notification_text; ?>

										</div>
										<li class="divider"></li>
										<div class="notification-footer" style="background-color: #3f6c9c;text-align: center;">
											<h4 class="menu-title"><a style="color: black !important;" href="<?php echo $site_url; ?>/app/customer/selfserviceview/notification/1">View all</a><i class="glyphicon glyphicon-circle-arrow-right"></i></h4>
										</div>
									</ul>
								</div>
							</rn:condition>
						...
						<script type="text/javascript">
							...
							if (contactid) {
								...
								var BounceNotifications6 = [];
								var BounceNotifications7 = [];
								...
								$.post("/cc/AjaxCustom/agreements", {
									...
									.done(function(data) {
										...
										console.log("agrements api", JSON.parse(data));
										...
										if (dataa.length > 0) {
											for (var i = 0; i < dataa.length; i++) {
												...
												// 	})
												// 	.done(function(data) {
												// 		console.log('API response from PNTNotification standard.php', data);
												...
												// 		if (data.includes('APPLICANT_NAME')) {
												// 			console.log("inside rc pedning not");
												// 			// d='';
												// 			d = JSON.parse(data);
												// 			BounceNotifications7.push(d);
												// 			document.getElementById('notification-window').innerHTML += "<div class='notification-item-read'><h5 class='item-title'>Hi " + d[0]['APPLICANT_NAME'] + ",Your Post disbursal document (PDD) is pending  since " + d[0]['FILE_AUTHORISATION_DATE'] + " .Please mail a copy of your vehicle registration certificate (RC) to helpdesk@tvscredit.com .</h5></div><li class='divider'></li>"
												// 			// d=JSON.parse(d);
												...
												// 			// d='';
												// 			d = JSON.parse(data);
												// 			console.log("firstname", d[0]['FIRSTNAME']);
												// 			BounceNotifications6.push(d);
												// 			document.getElementById('notification-window').innerHTML += "<div class='notification-item-read'><h5 class='item-title'>Hi " + d[0]['FIRSTNAME'] + ",Your TVS two-wheeler insurance has been renewed.<a id = 'id_insurance_notify_2' href='https://tvscs.custhelp.com/app/customer/selfserviceview/instrumentdetails/1' style='color:blue; padding:5px;'>Click here to download the insurance document</a></h5></div><li class='divider'></li>"
												// 			// d=JSON.parse(d);

												// 			$('#id_insurance_notify_2').click(function() {
												// 				//Some code
												// 				console.log("i am clicked");
												// 				localStorage.setItem("insu_noti_2", "1");
												// 			});
												...
												// 			document.getElementById('notification-window').innerHTML += "<div class='notification-item-read'><h5 class='item-title'>Hi " + d[0]['FIRST_NAME'] + ",Your EMI of Rs. " + d[0]['EMI'] + " for the month of " + d[0]['DUE_MONTH_YEAR'] + " has bounced due to insufficient balance in your account. Please pay immediately to avoid penalty charges.  <a href='https://pg.tvscredit.com/Home/Index' style='color:black; padding:0px;'>Pay now!</a>. Please ignore if it is already paid.</h5></div><li class='divider'></li>"
												// 			// d=JSON.parse(d);
												...
												$.post("/cc/AjaxCustom/WelcomeLetterNotification", {
													...
													.done(function(data) {
														if (data.includes('AGMTNO')) {
															d = JSON.parse(data);
															BounceNotifications5.push(d);
															document.getElementById('notification-window').innerHTML += "<div class='notification-item-read notice info'><h5 class='item-title'>Welcome to the TVS Credit family! Hi " + d[0]['FIRST_NAME'] + ",Your welcome kit for loan a/c no. " + d[0]['AGMTNO'] + ",containing key information has been sent to your registered email id.View your loan details here!<a href='https://tvscs.custhelp.com/app/customer/dashboard' style='color:black; padding:0px;'>https://tvscs.custhelp.com/app/customer/dashboard</a></h5></div><li class='divider'></li>"
															// d=JSON.parse(d);
							...
							<ul class="dropdown-menu slide-animate" style="margin-right:2px;">
								<li>
									<div class="contact-info">
										<div class="contacts">044-66123456
											<p id="a"><img src="/euf/assets/themes/standard/Images-new/mobile_icon/contact-us icon.png">Contact Us:
												<a href="callto://+044-66123456"> 044-66-123456</a>, <a href="callto://+18001035005"> 1800 103 5005</a>
											</p>
										</div>
									</div>
								</li>
								...
								<li>
									<div class="contact-info text-capitalize">
										<div class="contacts">
											<p class="color_text"> <a href="http://183.87.127.174:8484/careers/">careers</a></p>
										</div>
									</div>
								</li>
							</ul>
				...
				<nav id="nav-menu-container" class="hidden-xs ">
					<div class="row">
						<div class="contact-info">
							<div class="contacts">
								<p><img src="/euf/assets/themes/standard/Images-new/contact-us icon.png">Contact Us: <a href="callto://+044-66123456">044-66-123456</a>,<a href="callto://+18001035005"> 1800 103 5005</a></p>
							</div>
						</div>
						<div class="contact-info">
							<div class="contacts">
...
<div class="contacts"><p> <a href="http://183.87.127.174:8484/careers/">careers</a></p>  </div>
		...
		<rn:condition logged_in="true">
			<?php
			//IARC CR - added value part in below condition
			if ($userProfile['cType'] == 'Customer' && $value != "1") { ?>
				<header>
					<style type="text/css">
						...
						<div class="rn_NavigationBar">
							...
							<i class="fa fa-google-wallet" aria-hidden="true"></i>&nbsp;<a href="<?php echo $site_url; ?>/app/customer/wallet">Wallet</a>
								...
								<li>
									...
									<rn:widget path="navigation/NavigationTab" label_tab="Ask a Question" link="/app/raisequery" pages="raisequery, ask_confirm" />
								</li>
								<!--<li>
	...
	<!-- IARC -->
	<?php } elseif( $userProfile['cType'] == 'Customer' && $value == "1" ) { ?>
