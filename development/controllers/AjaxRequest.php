 ...
 */
final class AjaxRequest extends \RightNow\Controllers\Base
{
    public function __construct()
    {
        parent::__construct();

        parent::_setClickstreamMapping(
            array(
        ...
        parent::_setMethodsExemptFromContactLoginRequired(
            array(
                'getNewFormToken',
                'sendForm',
    ...
    public function deleteNotification()
    {
        AbuseDetection::check();
        if (!$this->checkForValidFormToken()) {
            $this->_renderJSON(array('error' => Config::getMessage(ERROR_REQUEST_ACTION_COMPLETED_MSG)));
            return;
        }
    ...
    public function addOrRenewNotification()
    {
        ...
        if (!$this->checkForValidFormToken()) {
            ...
            $error = Config::getMessage(ERROR_REQUEST_ACTION_COMPLETED_MSG);
        } else {
        ...
        $this->_renderJSON(
            array(
                'error' => $error,
                'notifications' => Framework::sortBy($notifications, true, function ($n) {
                    return $n['rawStartTime'];
                }),
    ...
    public function addSocialSubscription()
    {
        AbuseDetection::check();
        if (!$this->checkForValidFormToken()) {
            $this->_renderJSON(array('errors' => Config::getMessage(ERROR_REQUEST_ACTION_COMPLETED_MSG)));
            return;
        }
    ...
    // public function nocupdatecreatedbycontact(){
    //     $this->model('custom/Incident')->updatenocreatebycontact();
    // }
    ...
    public function sendForm()
    {
        ...
        $this->_echoJSON($this->model('custom/Field')->sendForm($data, $listOfUpdateRecordIDs ?: array(), ($smartAssistant === 'true'))->toJson());
    }
     ...
     * form submission, including errors, IDs of records created, or SA results if an incident is being submitted.
     */
    public function sendFormEmpIncUpdate()
    {
        AbuseDetection::check($this->input->post('f_tok'));
        ...
        $this->_echoJSON($this->model('custom/Field')->sendFormEmpIncUpdate($data, $listOfUpdateRecordIDs ?: array(), ($smartAssistant === 'true'))->toJson());
    }
