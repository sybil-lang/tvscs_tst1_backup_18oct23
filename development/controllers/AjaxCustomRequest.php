 ...
 */
class AjaxCustomRequest extends \RightNow\Controllers\Base
{
    /**
    ...
    private function _getHierValues($id, $filter, $linking)
    {
		...
		$msg=\RightNow\Connect\v1_3\MessageBase::fetch(CUSTOM_MSG_EmpPortal_EmployeeHelpdesk);
		$report_id= $msg->Value;
		$filter = array();
	...
	//	if(!empty($results->result)){
			$resultsArray = array();
			foreach($results->result as $key => $response){
				foreach($response as $resultData){
					if(empty($resultData['hasChildren']) || !$resultData['hasChildren']){
						$key = array_search($resultData['label'], array_column($report_result, 'Category'));
					//echo $key."good";
						if($key === false){
		...
		/*if(!empty($resultsArray)){
				$resultsArray = array($resultsArray);
				$responseObject = new ResponseObject('is_array');
				$responseObject->result = $resultsArray;
