...
class DealerCustom extends \RightNow\Controllers\Base
{
	...
	function getTaReturnResult()
	{


		$tr_api_url = RNCPHP\MessageBase::fetch("CUSTOM_MSG_TA_RETURN_DATA_API_URL")->Value;

		//'https://payment.tvscredit.com/Customer/TADueRequest',
		//'https://pguat.tvscredit.com/Customer/TADueRequest',

		$dealerCode = $_REQUEST['dealer_code'];
		load_curl();
		$curl = curl_init();
		// echo $dealerCode; exit();
		curl_setopt_array($curl, array(
			CURLOPT_URL => $tr_api_url,
		...
		// if($response_decoded->ReturnMessage == "" || $response_decoded->ReturnMessage == "Cannot able to get Trade Advance details. Please contact administrator.")
		// {
		// 	echo null;
		// }
		// else{
		// 	echo json_encode($response_decoded->ReturnList);
		// }
		$arrayName = array("ReturnMessage" => $response_decoded->ReturnMessage, "ReturnList" => $response_decoded->ReturnList);
		$json_data = json_encode($arrayName);
		echo $json_data;
	}

	function getTApaymentURI()
	{
		// print_r($_POST);
		//{"AgreementNo": "MH3029TA00031", "AgreementStatus": "L", "ChannelCode": "DAP", "ChargeType": "TDA", "CustomerName": "JAIMAL AUTOMOBILES PVT LTD", "EmiOverdue": "104287.78", "FinalAmount": "509.78", "MaxSOFVal": "104287", "PaymentID": "DAP686988", "SOFFlag": "N"}

		//Array ( [AgreementNo] => MH3029TA00031 [AgreementStatus] => L [CustomerName] => JAIMAL AUTOMOBILES PVT LTD [EmiOverdue] => 491199.47 [FinalAmount] => 491199.47 [PaymentID] => DAP7215 )

		$tr_pay_api_url = RNCPHP\MessageBase::fetch("CUSTOM_MSG_TA_RETURN_PAY_API_URL")->Value;

		//"https://payment.tvscredit.com/Customer/CustomerRequest",
		//"https://pguat.tvscredit.com/Customer/CustomerRequest",

		$AgreementNo = $_POST['AgreementNo'];
		$AgreementStatus = $_POST['AgreementStatus'];
		$CustomerName = $_POST['CustomerName'];
		$EmiOverdue = $_POST['EmiOverdue'];
		$FinalAmount = $_POST['FinalAmount'];
		$PaymentID = $_POST['PaymentID'];
		$ChannelCode = "DAP";
		$ChargeType = "TDA";
		$SOFFlag = "N";
		$MaxSOFVal = $EmiOverdue;

		try {
			$postString = '{"AgreementNo": "' . $AgreementNo . '", "AgreementStatus": "' . $AgreementStatus . '", "ChannelCode": "' . $ChannelCode . '", "ChargeType": "' . $ChargeType . '", "CustomerName": "' . $CustomerName . '", "EmiOverdue": "' . $EmiOverdue . '", "FinalAmount": "' . $FinalAmount . '", "MaxSOFVal": "' . $MaxSOFVal . '", "PaymentID": "' . $PaymentID . '", "SOFFlag": "' . $SOFFlag . '"}';
			// echo $postString;

			load_curl();
			$curl = curl_init();
			curl_setopt_array($curl, array(
				CURLOPT_URL => $tr_pay_api_url,
				CURLOPT_RETURNTRANSFER => true,
				CURLOPT_ENCODING => "",
				CURLOPT_MAXREDIRS => 10,
				CURLOPT_TIMEOUT => 100,
	...
	function GetWCInsuranceResult()
	{
		...
		$curl = curl_init();
		curl_setopt_array($curl, array(
			CURLOPT_URL => "https://tvscscrmservice.tvscredit.com/CRMService.svc/GetWCInsuranceExistDealer", //UAT: "https://tvscscrmservice.tvscredit.com/CRMService.svc/GetWCInsuranceExistDealer"
			CURLOPT_RETURNTRANSFER => true,
			CURLOPT_ENCODING => "",
			CURLOPT_MAXREDIRS => 10,
			CURLOPT_TIMEOUT => 0,
	...
	function getWCPOPUP()
	{
		...
		curl_setopt_array($curl, array(
			CURLOPT_URL => "https://tvscscrmservice.tvscredit.com/CRMService.svc/ShowWCPOPUPWindow",
			CURLOPT_RETURNTRANSFER => true,
			CURLOPT_ENCODING => "",
			CURLOPT_MAXREDIRS => 10,
			CURLOPT_TIMEOUT => 0,
	...
	function setWCPOPUP()
	{
		...
		curl_setopt_array($curl, array(
			CURLOPT_URL => "https://tvscscrmservice.tvscredit.com/CRMService.svc/SaveWCPOPUPDetails",
			CURLOPT_RETURNTRANSFER => true,
			CURLOPT_ENCODING => "",
			CURLOPT_MAXREDIRS => 10,
			CURLOPT_TIMEOUT => 0,
	...
	function getProductsName()
	{
		...
		curl_setopt_array($curl, array(
			CURLOPT_URL => "https://leadsservice.tvscredit.com/LMSservice.svc/GetProductName",
			CURLOPT_RETURNTRANSFER => true,
			CURLOPT_ENCODING => "",
			CURLOPT_MAXREDIRS => 10,
			CURLOPT_TIMEOUT => 100,
    ...
    */
		...
		//print_r($params);
		$LMS_result = soap_lead_rest_call('InsertLMSDataNew', $params, $url);
		...
		header('Content-type:application/json;charset=utf-8');
		echo json_encode($LMS_result['InsertLMSDataNewResult']);
	...
	function getSalesDisbursedCount()
	{
		...
		//$url = "https://tvscscrmuat.tvscredit.com:8180/tvscrmlosws/rest/GetTVSDetails/giveOutput?hidInputObj=";
    ...
    */
	function getDealerDisbursalRestData()
	{
		...
		//      curl_setopt_array($curl, array(
		//   CURLOPT_URL => "https://customeroperationapi.tvscredit.com/LoanDetails/GetDealerDisbursal",
		//   CURLOPT_RETURNTRANSFER => true,
		//   CURLOPT_ENCODING => "",
		//   CURLOPT_MAXREDIRS => 10,
		//   CURLOPT_TIMEOUT => 0,
		...
		if ($err) {
			...
			if ($response->error_code == "1001" || $response == null) {
				...
				$true_array = json_decode($response, true);
				print_r(json_encode($true_array["ReturnOutput"], true));
			}
		}
	}
	...
	function reloadDealerDisbursalRestData()
	{
		...
		//  curl_setopt_array($curl, array(
		//    CURLOPT_URL => "https://tvscscrmservice.tvscredit.com/CRMService.svc/GetDealerDisbursalDetails",
		//    CURLOPT_RETURNTRANSFER => true,
		//    CURLOPT_ENCODING => "",
		//    CURLOPT_MAXREDIRS => 10,
		//    CURLOPT_TIMEOUT => 30,
	...
	function getPaymentRestData()
	{
		...
		$curl = curl_init();
		curl_setopt_array($curl, array(
			CURLOPT_URL => "https://tvscscrmservice.tvscredit.com/CRMService.svc/GetDealerPaymentDetails",
			CURLOPT_RETURNTRANSFER => true,
			CURLOPT_ENCODING => "",
			CURLOPT_MAXREDIRS => 10,
			CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
	...
	function reloadSalesRestData()
	{
		...
		if ($methodName == 'getDealerTATransaction') {
			...
			$curl = curl_init();
			curl_setopt_array($curl, array(
				CURLOPT_URL => "https://tvscscrmservice.tvscredit.com/CRMService.svc/getDealerTATransaction",
				CURLOPT_RETURNTRANSFER => true,
				CURLOPT_ENCODING => "",
				CURLOPT_MAXREDIRS => 10,
				CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
		...
		elseif ($methodName == 'getTADealerSummary') {
			...
			////----------------Rahul changes start (18-04-2023)

			$curl = curl_init();

			curl_setopt_array($curl, array(
				CURLOPT_URL => 'https://dealerportaluat.tvscredit.com/DealerPortalMobileServices/dealerapi/getTATranSummary',
				CURLOPT_RETURNTRANSFER => true,
				CURLOPT_ENCODING => '',
				CURLOPT_MAXREDIRS => 10,
				CURLOPT_TIMEOUT => 0,
				...
				CURLOPT_POSTFIELDS => '{
					...
					"FromDate":"' . date('d-M-Y', mktime(0, 0, 0, $smonth, $sday, $syear)) . '",
					
					"ToDate":"' . date('d-M-Y', mktime(0, 0, 0, $emonth, $eday, $eyear)) . '",
				...
				',
				CURLOPT_HTTPHEADER => array(
					'Authorization: Basic REVBTEVSUE9SVEFMOkRFQUxFUlBPUlRBTCQ=',
					'Content-Type: application/json',
					'Cookie: JSESSIONID=lPhNpfy0MPhBLcXMFPECoe6wa5qs3jdNL_tgCB0I.csdcapps9'
				),
			));
			$response = curl_exec($curl);
...
*/
    ...
    */
	function getTARestData()
	{
		...
		$methodName = $_POST['method'];
		$year = date("Y", strtotime($startDate));
		//oldest url: $url = "https://tvscscrmuat.tvscredit.com:8180/tvscrmlosws/rest/GetTVSDetails/giveOutput?hidInputObj=";
		...
		// $url = "https://tvscscrm.tvscredit.com/tvscrmlosws/rest/GetTVSDetails/giveOutput?hidInputObj=";
		$url = "https://tvscscrmservice.tvscredit.com/CRMService.svc/getTARestData";
		$response = array();
		// $data = array(
		// 	"methodName" => "$methodName",
		// 	"dealerCode" => $dealerCode,
	...
	function createTARequest()
	{
		...
		*/
		try {
			...
			$filters[0] = $filter;
			// print_r($filters);
			$ar = \RightNow\Connect\v1_3\AnalyticsReport::fetch(100877);
	...
	function createWCRequest()
	{
		...
		try {
			...
			if (count($invoicesArr) > 0) {
				load_curl();
				$dealerInvoicesUrl = "https://tvscscrmservice.tvscredit.com/CRMService.svc/Eligiblefordealer_invoicedetails";
	...
	*/
	...
	function sendApproverEmailRequest()
	{
		...
		if (!empty($incident_id)) {
			...
			curl_setopt_array($curl, array(
				CURLOPT_URL => "https://tvscscrmservice.tvscredit.com/CRMService.svc/getTARequestEmailIds",
				CURLOPT_RETURNTRANSFER => true,
				CURLOPT_ENCODING => "",
				CURLOPT_MAXREDIRS => 10,
				CURLOPT_TIMEOUT => 180,
		...
		if ($err) {
			...
			for ($i = 0; $i < count($responseArr); $i++) {
				if ($responseArr[$i]["Year"] == $year && $responseArr[$i]["Month"] == $month) {
					...
					$url = "https://tvscscrmservice.tvscredit.com/CRMService.svc/GetCDDealerInvoice/" . $downloadId;
					echo $url;
					break;
				}
			}
