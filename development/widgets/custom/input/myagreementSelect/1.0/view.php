...
<div>
	...
	<select id="rn_<?= $this->instanceID ?>_<?= $this->data['attrs']['name'] ?>" class="rn_BasicSelection setp" name="<?= $this->data['attrs']['name'] ?>" required="true">
	...
	foreach($this->data['agreement_arr'] as $key => $response){?>
					<option value="<?= $response['Loan_ID'] ?>"><?= $response['Agreement No'] ?></option>
	<?php
	}
	?>
