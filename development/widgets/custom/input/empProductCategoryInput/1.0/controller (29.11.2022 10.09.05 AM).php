...
use \RightNow\Utils\Url,
    \RightNow\Utils\Text;
class empProductCategoryInput extends \RightNow\Widgets\ProductCategoryInput {
	...
	//	unset($hierData);
	/*$msg=\RightNow\Connect\v1_3\MessageBase::fetch(CUSTOM_MSG_Employee_Category);
	$report_id=$msg->Value;
	...
	if($report_id>0){
			...
			//print_r($cdata); 
			$hierData[] = array('id' => 0, 'label' => 'Select a Category');
			$default = $this->data['attrs']['default_value'];
		//echo "good";
			foreach($report_result as $key => $response){
					if($default == $response['Category_Id']){
							$hierData[] = array('id'=> $response['Category_Id'], 'label' => addslashes($response['Category']),'hasChildren' => '0', 'selected' => 'true');
					}else{
							$hierData[] = array('id'=> $response['Category_Id'], 'label' => addslashes($response['Category']),'hasChildren' => '0', 'selected' => '');
					}
			}
			$defaultHierMaps[0] = $hierData;
	}*/
	//	echo '<pre>';
        $report_cats = array();
        $report = \RightNow\Connect\v1_3\AnalyticsReport::fetch("100549");
        $report_results = $report->run();
        $nrows = $report_results->count();
        if($nrows>0)
                {
                     $row = $report_results->next();
                     for ( $ii = 0; $ii++ < $nrows; $row = $report_results->next() ) 
                     {
                        array_push($report_cats, $row['Category Level 1 ID']);
                           
                     }
                }
		$i = 0;
		$defaultHierMaps[0][] = array('id' => 0, 'label' => 'Select a Category');
		foreach($defaultHierMap[0] as $data){
		//print_r($data);
	//print_r( $data[$i]); echo $i."--";
				// if($data['id'] == '97' || $data['id'] == '879' || $data['id'] == '965' || $data['id'] == '1421' || $data['id'] == '1654' || $data['id']== '1813'){
				// 		$defaultHierMaps[0][] = $data;
				// }
                if(in_array($data['id'], $report_cats)){
                    $defaultHierMaps[0][] = $data;
                } 
				$i = $i+1;
		}
		//print_r($defaultHierMaps);
        $this->data['js']['hierData'] = $defaultHierMaps;
    }
