...
class ProductCategoryInputEmpCustomer extends \RightNow\Widgets\ProductCategoryInput {
    ...
    function getData() {
        ...
        $this->data['js']['linkingOn'] = $this->data['attrs']['linking_off'] ? 0 : $this->CI->model('custom/Prodcat')->getLinkingMode();
        ...
        if($this->data['js']['linkingOn'] && !$isProduct) {
            $defaultProductID = $this->CI->model('custom/Prodcat')->getDefaultProductID() ?: null;
            $this->data['js']['link_map'] = $defaultHierMap = $this->CI->model('custom/Prodcat')->getFormattedTree($dataType, $defaultChain, true, $defaultProductID, $maxLevel, $this->data['attrs']['report_id'])->result;
            // echo "<pre>";
            // print_r($defaultHierMap);
            $this->data['js']['hierDataNone'] = $this->CI->model('custom/Prodcat')->getFormattedTree($dataType, array(), true, null, $maxLevel, $this->data['attrs']['report_id'])->result;
            ...
            if($isProduct) {
                $this->CI->model('custom/Prodcat')->setDefaultProductID(end($defaultChain));
            }
            $defaultHierMap = $this->CI->model('custom/Prodcat')->getFormattedTree($dataType, $defaultChain, false, null, $maxLevel, $this->data['attrs']['report_id'])->result;
            // echo ($this->data['attrs']['report_id']);
            // echo "<pre>";
            // print_r($defaultHierMap);
        }
        ...
        if($this->data['attrs']['verify_permissions'] !== 'None') {
            ...
            $permissionedHierarchy = $this->CI->model('custom/Prodcat')->$permissionMethod($isProduct)->result;
    ...
    protected function getDefaultChain() {
        ...
        if($defaultValue) {
            ...
            $defaultChain = (count($defaultChain) === 1)
                ? $this->CI->model('custom/Prodcat')->getFormattedChain($dataType, $defaultChain[0], true)->result
                : $this->CI->model('custom/Prodcat')->getEnduserVisibleHierarchy($defaultChain)->result;
