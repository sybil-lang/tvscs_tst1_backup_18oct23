RightNow.namespace( 'Custom.Widgets.input.empFormSubmit' );
Custom.Widgets.input.empFormSubmit = RightNow.Form.extend( {
    /**
    ...
    _fireSubmitRequest: function () {
        ...
        if ( jQuery( "#agreement_no" ).length ) {
            if ( jQuery( "#agreement_no" ).val().trim().length == 0 ) {
                errorMessage = "Please enter an Agreement Number";
    ...
    _onFormValidated: function () {
        ...
        var dpaddress = jQuery( "textarea.rn_TextArea" ).val();
        // var imei = jQuery( "input.rn_Number" ).val();
        // var imei = jQuery( "textarea.rn_TextArea" );
        // var imei = jQuery( "#rn_TextInput_33_Incident.CustomFields.c.imei_number" ).val();
        // const textarea = document.querySelector( 'textarea' );
        // alert( imei )

        // alert( imei.length )
        jQuery( "select[name='Incident.Category']" ).val();
        var address_type = jQuery( "select[name='Incident.CustomFields.c.preferred_address'] option:selected" ).text();
        address_type = ( address_type != "" && address_type != undefined ) ? address_type.trim() : address_type;
        ...
        if ( sel_val.includes( 'Consumer Finance' ) ) {
            ...
            // if ( /^\d{15}$/.test( imei ) ) {
            if ( imei.length == 15 ) {
                // alert( 'valid 15 digit' );
                this._toggleLoadingIndicators( true );
                console.log( this.getValidatedFields() );
                this.fire( "send", this.getValidatedFields() );
            }
            if ( imei.length < 15 ) {
                // alert( 'not a valid 15 digit' );
                this._toggleClickListener( true );
                window.stop();
                throw new Error( "ERROR" );
            }
        ...
        if ( $( '#category_name' ).val() == "WC Enrollment Doc Submission" ) {
            is_selected = true;
        }
        ...
        if ( $( '#dealercode' ).length ) {
            dealerID = document.getElementById( 'dealercode' ).value;
            // alert(FattachLength+" and is_selected:"+is_selected);
        }
        // if(is_selected==false){
        //     this._toggleLoadingIndicators(true);
        //     if(loanID !=''){
        //         this.data.attrs.add_params_to_url = "loanid/"+loanID;
        //     }else if(dealerID != ''){
        //         this.data.attrs.add_params_to_url = "dealerid/"+dealerID;
        //     }
        //     this.fire("send", this.getValidatedFields());
        // }//
        if ( is_selected == false ) {
            ...
            if ( address_type == "New Address" ) {
                if ( FattachLength > 0 ) {
                    if ( pincode != "" && pincode != undefined && pincode != null ) {


                        // return false;
                        if ( dpaddress.length == 0 ) {
                            this._toggleClickListener( true );
                            window.stop();
                            throw new Error( "ERROR" );
                        }
                    ...
                    else if ( dpaddress.length == 0 ) {
                        alert( "Dispatch Address is required!" );
                        this._toggleClickListener( true );
                        window.stop();
                        throw new Error( "ERROR" );
                    }
                    ...
                    alert( "Kindly upload an address proof document!" );
                    this._toggleClickListener( true );
                    window.stop();
                    throw new Error( "ERROR" );
            ...
            else if ( address_type == "Branch Address" ) {
                if ( dpaddress != "" && dpaddress != undefined && dpaddress != null ) {
                    this._toggleLoadingIndicators( true );
                    this.fire( "send", this.getValidatedFields() );
                }
                else {
                    alert( "Kindly add the Branch Name in Dispatch Address!" );
                    this._toggleClickListener( true );
                    window.stop();
                    throw new Error( "ERROR" );
                }
            ...
            if ( FattachLength > 0 ) {
                ...
                alert( "Kindly Upload Documents and try again." );
                this._toggleClickListener( true );
                window.stop();
                throw new Error( "ERROR" );
            }
    ...
    _displayErrorMessages: function ( messageArea ) {
        ...
        if ( !this.Y.DOM.inViewportRegion( this.Y.Node.getDOMNode( messageArea ), true ) ) {
            ( new this.Y.Anim( {
                ...
                to: { scrollTop: messageArea.get( 'offsetTop' ) - 40 },
                duration: 0.5
            } ) ).run();
        }
    ...
    _navigateToUrl: function ( result ) {
        ...
        if ( result.redirectOverride ) {
            ...
            var paramsToAdd = '';
            var successurl = this.data.attrs.on_success_url;
            //	alert(successurl);
            this.Y.Object.each( result.transaction, function ( details ) {
                if ( details.key && successurl == '/app/employee/customer_ask_confirm' ) {
                    paramsToAdd += '/ci_id/' + details.value;
                } else if ( details.key && successurl == '/app/employee/dealer_ask_confirm' ) {
                    paramsToAdd += '/di_id/' + details.value;
                }
                else {
