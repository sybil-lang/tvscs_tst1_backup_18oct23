RightNow.namespace('Custom.Widgets.login.loginSession');
Custom.Widgets.login.loginSession = RightNow.Widgets.extend({ 
   /**
     * Widget constructor.
     */
    constructor: function() {
		...
		this._mobilenumberField = this.Y.one(this.baseSelector + "_MobileNumber");
     ...
     * @param {Boolean} turnOn Whether to add or remove the loading indicators.
     */
    _toggleOtpLoading: function(turnOn) {
//		alert(this.baseSelector);
        this._widgetContent || (this._widgetContent = this.Y.one(this.baseSelector + '_ThirdPartyLogin'));
...
/**
    ...
    _onSubmitOTP: function(e)
    {
        ...
        if(RightNow.Event.fire("evt_otpFormSubmitRequest", eventObject)){
            this._toggleOtpLoading(true);

            
			//alert(this.data.attrs.login_ajax);
            RightNow.Ajax.makeRequest(this.data.attrs.sendotp_ajax, eventObject.data, {successHandler: this._onOtpResponse, scope: this, data: eventObject, json: true});
