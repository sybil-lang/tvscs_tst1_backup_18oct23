RightNow.namespace('Custom.Widgets.login.EmailOtp');
Custom.Widgets.login.EmailOtp = RightNow.Widgets.extend({     /**
    /**
     * Widget constructor.
     */
    constructor: function() {
        ...
        this._usernameField = this.Y.one(this.baseSelector + "_Email");
        this._passwordField = this.Y.one(this.baseSelector + "_Otp");
		this._emailaddressField = this.Y.one(this.baseSelector + "_Email");
...
/**
    ...
    _onSubmitOTP: function(e)
    {
        ...
        var emailaddress = (this._emailaddressField) ? this.Y.Lang.trim(this._emailaddressField.get("value")) : "",
            errorMessage, eventObject;
        ...
        if(errorMessage)
        {
            this._addOtpErrorMessage(errorMessage, this.baseDomID + '_Mobile');
            return false;
        }
    ...
    _onOtpResponse: function(response, originalEventObject)
    {
      ...
      console.log(response);
        if(response.success)
        {
            ...
            //this.Y.one(this.baseSelector + "_Content").set("innerHTML", response.message);
			this._addOtpErrorMessage(response.message, this.baseDomID + '_Email', false);
           /* var redirectUrl = this._getRedirectUrl(response);
            if(this.Y.UA.ie && this.Y.UA.ie < 9 && RightNow.Text.beginsWith(redirectUrl, '/ci/fattach/get/'))
                this.Y.one(this.baseSelector).set('innerHTML', RightNow.Text.sprintf(RightNow.Interface.getMessage("PLS_CLCK_HREF_EQS_PCT_S_THAN_S_MSG"), redirectUrl));
            else
        ...
        else
        {
            ...
            this._addOtpErrorMessage(response.message, this.baseDomID + '_Email', response.showLink);
        }
